jQuery(function(){
	...
	CarregaArquivos = function(parent){
		var tipoReplace = ( parent == 'video' ) ? '{urlFILE}' : '{urlFILE}',
			...
			markup_img  = '<li id="{idFILE}" class="boxPreview"><span><span class="{bgTIPO}"><img src="'+ tipoReplace +'" width="114" height="85" alt="preview {tituloFILE}" /></span></span>{VISIVEL}<button class="icone editar" title="Editar arquivo">Editar</button><button class="icone excluir" title="Excluir arquivo">Excluir</button></li>',
			markup_axo  = '<li id="{idFILE}" class="lineAnexo"><span></span><div>\
								<img width="32" height="32" alt="" src="'+ siteURL +'/ack-core/images/icones/extension_{extFILE}.png"><p>{titleFILE}<em>{nameFILE} - <b>{sizeFILE}</b></em></p>\
			...
			id          = main.objPage.id,
			modulo      = main.objPage.modulo,
			pedido      = JSON.stringify({ 'acao':'carregarArquivos','tipo':parent, 'id':id, 'modulo':modulo });

		$.ajax({
			/*url: siteURL+'/ack/'+main.objPage.arquivoRetorno+'/',*/
			url:   main.url+'/ack/'+main.objPage.arquivoRetorno+'/routerAjax',
			type: 'POST',
			data: {'ajaxACK':pedido},
			dataType: 'json',
				...
				if( data.status === '1' ){
					if( $(thisParent).hasClass('arquivosBloco') ){
						$.each(data.galeria, function(i_ac, val_ca){
							var icone = '<i>imagem</i>';

							if ( val_ca.cover != null ) {
								icone = '<label class="checkCover"><span class="inputCover '+ ((val_ca.cover == 1) ? 'ok' : '') +'"><input type="checkbox" name="imgCover" '+ ((val_ca.cover == 1) ? 'checked="checked"' : '') +' /></span> Capa</label>';
							};
	...
	$('.arquivosBloco').on('click', '.inputCover input', function(){
		...
		if ( clicado.is(':checked') ) {
			...
			$.ajax({
				url:  main.url+'/ack/'+main.objPage.arquivoRetorno+'/salvarCover',
				type: 'POST',
				data: {'ajaxACK':pacote},
				dataType: 'json',
	...
	/* ----------------------------------------------------------------------------------------------------
	   ...
	   ---------------------------------------------------------------------------------------------------- */
	$('.upMidias').on('click', '.excluir', function(){
		...
		$.ajax({
			/*url: siteURL+'/ack/'+main.objPage.arquivoRetorno+'/',*/
			url:   main.url+'/ack/'+main.objPage.arquivoRetorno+'/excluirArquivo',
			type: 'POST',
			data: {'ajaxACK':pedido},
			dataType: 'json',
	...
	$('.upMidias').find('input[type="file"]').each(function(index, element){
		...
		switch(nameButton){
			case 'imagem':
				var extesoes  = '*.jpeg;*.JPEG;*.jpg;*.JPG;*.gif;*.png;*.PNG',
					diretorio = '/galeria';
			break;
			case 'video':
		...
		input.uploadify({
			'uploader' : siteURL+'/vendor/uploads/flash/uploadify.swf',
			'script'   : siteURL+'/vendor/uploads/uploadify.php',
			'buttonImg': siteURL+'/ack-core/images/btnUpload_'+ nameButton +'.png',
			'width'    : 395,
			'height'   : 70,
			'fileExt'  : extesoes,
			...
			'onSelectOnce':      function( event ){
				...
				$.ajax({
					url:  siteURL+'/ack/'+main.objPage.arquivoRetorno+'/routerAjax',
					type: 'POST',
					data: {'ajaxACK':pacote},
					dataType: 'json',
			...
			'onProgress':        function( event, ID ){
				$('#imagem_upload'+ID).find('.icone.cancel').css({ opacity:1 });
				$('#imagem_upload'+ID).find('.uploadifyProgress').css({ opacity:1 });
			},
			...
			'onAllComplete':     function( event ){
				...
				var pedido = JSON.stringify({
						...
						'modulo':   main.objPage.modulo,
						'id':       main.objPage.id.toString(),
						'multi':    String(multi)
					});

				$.ajax({
					/*url: siteURL+'/ack/'+main.objPage.arquivoRetorno+'/',*/
					url:   main.url+'/ack/'+main.objPage.arquivoRetorno+'/enviarArquivos',
					type: 'POST',
					data: {'ajaxACK':pedido},
					dataType: 'json',
					...
					success: function(data){
						...
						if ( main.objPage.modulo == 'img_image' ) {
	...
	$(document).on('click', '.upFile', function(){
		...
		if( $('.listaArquivos').is(':visible') ){
			...
			var pedido = JSON.stringify({ 'acao':'cadastroUrl', 'tipo':'video', 'idiomas':idiomas, 'url':novaURL.val(), 'tipoURL':tipoURL, 'id':idConteudo, 'modulo':modulo });
			$.ajax({
				url: siteURL+'/ack/'+main.objPage.arquivoRetorno+'/routerAjax',
				type: 'POST',
				data: {'ajaxACK':pedido},
	...
	$('.upMidias').on('click', '.cancelarUp', function(){
	...
	/* ----------------------------------------------------------------
	   ...
	   ---------------------------------------------------------------- */
	$('.includeURLvideo').bind('click', function(){
		var tipo   = $(this).val(),
			parent = $(this).parents('.collumA'),
			markup = '  <fieldset>\
							<img src="'+ siteURL +'/ack-core/images/icon_'+ tipo +'.jpg" width="54" height="21" alt="YouTube" /><br />\
	...
	$('.cancelarURLvideo').bind('click', function(){
		$('.tempBox').slideUp('fast', function(){
			$(this).children().remove();
			$('.lista_selecionados').slideDown('fast');
		});
	...
	/* ----------------------------------------------------------------
	   ...
	   ---------------------------------------------------------------- */
	$('.arquivosBloco').on('click', '.editar', function(){
		$('.menuAbas').find('button').attr('disabled', 'disabled');
		// ----- fecha e desabilita os outros modulos
		$('.upMidias').siblings('.modulo').find('.head > button').addClass('fechado').parents('.modulo').children('.slide').slideUp(function(){
			$(this).parents('.modulo').find('.head > button').attr('disabled', 'disabled');
		...
		if( $(this).parents('.contAba').attr('id') == 'abaIMAGEM' ){
			...
			$.ajax({
				/*url: siteURL+'/ack/'+main.objPage.arquivoRetorno+'/',*/
				url: main.url+'/ack/'+main.objPage.arquivoRetorno+'/editarArquivo',
				type: 'POST',
				data: {'ajaxACK':pedido},
				dataType: 'json',
		...
		// --------------- EDITAR VIDEOS
			...
			pedido = JSON.stringify({ 'id':idArquivo, 'modulo':main.objPage.modulo, 'acao':'editar', 'tipo':'video' });
			$.ajax({
				url: siteURL+'/ack/'+main.objPage.arquivoRetorno+'/',
				type: 'POST',
				data: {'ajaxACK':pedido},
				...
				success: function(data){
					if(data.status === '1'){
						...
						$('#upThumbnail').uploadify({
							'uploader' : siteURL+'/vendor/uploads/flash/uploadify.swf',
							'script'   : siteURL+'/vendor/uploads/uploadify.php',
							'buttonImg': siteURL+'/ack-core/images/btn_uploadFiles.png',
							'width'    : 102,
							'height'   : 28,
				...
				success: function(data){
					if( data.status === '1' ){
						...
						boxEdicao      = markup_anexo.replace(/{idiomas}/g, idiomas)
													 .replace(/{extICON}/g, siteURL+'/ack-core/images/icones/extension_'+extArquivo+'.png')
	...
	$('.upMidias').on('click', '.cancelarEdicao', function(){
		$('button').removeAttr('disabled');
	...
	$('.upMidias').on('click', '#salvarEdic_imagem', function(){
		$('button').removeAttr('disabled');
		...
		$.ajax({
			/*url: siteURL+'/ack/'+main.objPage.arquivoRetorno+'/',*/
			url: main.url+'/ack/'+main.objPage.arquivoRetorno+'/salvarArquivo',
			type: 'POST',
			data: {'ajaxACK':pacote},
			dataType: 'json',
	...
	$('#salvarEdic_video').bind('click', function(){
		$('button').removeAttr('disabled');
