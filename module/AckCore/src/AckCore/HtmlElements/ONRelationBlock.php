 ...
 * @link     http://github.com/zendframework/zf2 for the canonical source repository
 */
class ONRelationBlock extends RelationBlockAbstract
{
    ...
    public function defaultLayout()
    {
        ...
        $relatedModelInstance = new $relatedModelName;

        $relatedCall = $this->getRelatedColumnCall();
         //testa se já está setada a coluna da linha com relação externa
        if ($this->getCurrentRow()->$relatedCall()->getBruteVal()) {
            $where = (array("id"=>$this->getCurrentRow()->$relatedCall()->getBruteVal()));
        } else {
            $where = array();
        }
        ...
        if ($totalElements < 50) {
            $this->renderSelect($selectedRow);
        } else {
            $this->renderAutocomplete($selectedRow);
        }
    ...
    <?php
        ...
        ->setRelatedModelName($this->getRelatedModelName());
