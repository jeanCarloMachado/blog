...
use AckCore\Observer\Traits\Observable as ObservableTrait;
...
abstract class AbstractTableRowController extends AbstractActionController implements InjectApplicationEventInterface
{
    ...
    public function saveAjax()
    {
        ...
        $model = $this->getModelInstance();
        $id = (isset($this->ajax['id'])) ? $this->ajax['id'] : null;
        ...
        try {
            ...
            if (!empty($id)) {
                ...
                if (!isset($config['disableExceptionEncapsulation'])) {
                      try {
                            $result = $model->update($this->ajax['data'], $where);
                        } catch (\Exception $e) {
                ...
                if (!isset($config['disableExceptionEncapsulation'])) {
                    try {
                        $result = $model->create($this->ajax['data']);
                        if (is_object($result)) {
        ...
        if(is_object($result)) $result = $result->getId()->getBruteVal();
    ...
    public function loadItensAjax()
    {
        ...
        $params = array('limit'=>array('offset'=>$this->ajax['itensCount'], 'count'=> $this->itensPerPage));
